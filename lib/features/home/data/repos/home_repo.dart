...
//? Either<left,right> → right: if it goes good without errors , left: if goes error return that
...
{ //! repos pattern → make abstract class that we iside it make the featuers and methods that we will make and that tell what it will do?
  Future<Either<Failure,List<BookModel>>> fetchBestSellerBooks(); // todo: can make in abstract class only → must be rewrite in classes that extendes from it by @override
